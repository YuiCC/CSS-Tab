...
(function () {
    ...
    let pause = false
    let opt=null
    let ClockWidget = function (options) {
    ...
    function start(){
        intervalTimer = setInterval(function () {
            // if (!pause) { 
            //获取到canvas元素 
            clockCanvas = document.getElementById(opt.canvasId);
            //获取canvas中的画图环境  
            let ctx = clockCanvas.getContext("2d");

            drowClockLayout(opt, ctx);
            drawClockPoints(opt, ctx);
            // }  
        }, 1000);
    }
    ...
    function drowClockLayout(options, ctx) { // ctx :canvas.getContext
        //钟表的大小：初始值设置 
        ...
        for (let i = 1; i <= 60; i++) {
            try {
                ...
                if (i % 5 == 0) {
                    ...
                    ctx.fillText(getAlphabetHour(i / 5), 0, 0 - (clockSize - clockSize / 5));
                    ctx.fill();
    ...
    function drawClockPoints(options, ctx) { // ctx :canvas.getContext
        let clockSize = options.clockSize;
        //获取当前windows的时间 
        let now = new Date();
